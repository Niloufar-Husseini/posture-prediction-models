...
def train_loop(model,
  ...
  for inputs, targets in train_loader:
    ...
    len_true = torch.zeros(targets.shape[0], 2)
    len_pred = torch.zeros(targets.shape[0], 2)

    len_true[:, 0] = length_calc(inputs, 'LKNE', 'LANK', 'Left Thigh')
    len_true[:, 1] = length_calc(inputs, 'RKNE', 'RANK', 'Right Thigh')

    
    len_pred[:, 0] = length_calc(outputs, 'LKNE', 'LANK', 'Left Thigh')
    len_pred[:, 1] = length_calc(outputs, 'RKNE', 'RANK', 'Right Thigh')

    len_true = len_true.reshape(-1, 1, 2)
    len_pred = len_pred.reshape(-1, 1, 2)
...
def test_loop(model,
  ...
  with torch.no_grad():
    for inputs, targets in test_loader:
      ...
      len_pred[:, 0] = length_calc(test_outputs, 'LKNE', 'LANK', 'Left Thigh')
      len_pred[:, 1] = length_calc(test_outputs, 'RKNE', 'RANK', 'Right Thigh')
