...
def extract_frames_based_on_excel(excel_file, input_folder='', output_folder='extracted_frames'):
    ...
    for index, row in df_ranges.iterrows():
        ...
        try:
            ...
            df_csv = pd.read_csv(csv_file, skiprows=0, header=0)
            
            # Extract the specified frames (adjusting for 1-based vs 0-based indexing)
            extracted_frames = df_csv.iloc[start_frame:stop_frame+1]
            # Save to new CSV file with original header structure
            # with open(output_path, 'w') as f:
            #     # Write the original header lines
            #     f.writelines(header_lines)
            #     # Write the extracted frames
            extracted_frames.to_csv(f"extracted_frames/{row['File name']}_frames_{start_frame}_{stop_frame}_technique_{technique}_hand_{hand}.csv", index=False, lineterminator='\n')
...
if __name__ == "__main__":
    extract_frames_based_on_excel('frames.xlsx', r"C:\Niloofar\SCR\posture-prediction-models-main\external_data_processing\raw data\s6\converted")